...
import LraSchedulesDesign from "./LraSchedulesDesign";

// ── Types ─────────────────────────────────────────────────────────────────────

export interface LraEvent {
  id: string;
  name: string;
  image: string | null;
  date: string;
...
export interface LraJobListing {
  id: string;
  company: string;
  position: string;
  location: string;
...
export const LRA_EVENTS: LraEvent[] = [
  {
    id: "phates-lra",
    name: "Phates Local Recruitment Activity",
    image: "/assets/phates.jpg",
    date: "July 12, 2026",
    time: "8:00 AM – 5:00 PM",
    venue: "PESO Capiz Office",
  },
  {
    id: "capiz-tech-lra",
    name: "Capiz Tech Recruitment Drive",
    image: null,
    date: "August 5, 2026",
  ...
  {
    id: "western-visayas-lra",
    name: "Western Visayas Job Connect",
    image: null,
...
export const LRA_PLACEHOLDER_COLORS: Record<string, { bg: string; accent: string }> = {
  "capiz-tech-lra":      { bg: "#1a1d5e", accent: "#f5c842" },
  "western-visayas-lra": { bg: "#c0151a", accent: "#ffffff" },
};

// ── Data ──────────────────────────────────────────────────────────────────────

export const LRA_JOB_LISTINGS: LraJobListing[] = [
  {
    id: "phates-encoder",
    company: "Phates",
    ...
    requiredSkills: ["Computer Literacy", "Fast Typing", "Attention to Detail"],
  },
  {
    id: "phates-staff",
    company: "Phates",
    ...
    requiredSkills: ["Communication Skills", "MS Office Proficiency", "Organizational Skills"],
  },
  {
    id: "sample-sales",
    ...
    location: "Roxas City, Capiz",
    educationLabel: "High School Graduate",
    requiredSkills: ["Customer Service", "Product Knowledge"],
  },
];

export const LRA_EDUCATION_LEVELS = [
...
export const LRA_DEGREE_OPTIONS = [
  "BS Accountancy",
...
export const LRA_OFW_OPTIONS        = ["Yes", "No"];
export const LRA_FOURPS_OPTIONS     = ["Yes", "No"];
...
// ── Payload builder ───────────────────────────────────────────────────────────
// verificationToken is the signed HMAC token issued by LraSchedules.gs after
// a successful OTP check. doPost rejects the submission outright if this
// doesn't verify, so it's required here.

export function buildLraPayload(form: LraFormState, verificationToken: string) {
  return {
    formType:              "lra",
    verificationToken:     verificationToken,
    lraName:               form.lraName,
    lraDate:               form.lraDate,
    lraTime:               form.lraTime,
    lraVenue:              form.lraVenue,
...
export async function submitLra(
  form: LraFormState,
  verificationToken: string
): Promise<{ success: boolean; error?: string }> {
  try {
    const res = await fetch(LRA_SUBMIT_URL, {
      method:  "POST",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body:    JSON.stringify(buildLraPayload(form, verificationToken)),
    });
...
// Catches typo'd real domains (gmail.co, gmial.com, gmail.comm) that pass
// isEmail()'s format check but are almost certainly mistakes. Same approach
// as SraSchedules.tsx / JobFairSchedules.tsx.
...
/**
 * Flags a likely typo of a popular email domain (e.g. gmail.co, gmial.com,
 * gmail.con) — 1-2 edits away from a known provider, but not an exact match.
 * Returns the corrected email (with the popular domain swapped in), or null
 * if no likely typo is found.
 */
function findLikelyDomainTypo(email: string): string | null {
  const parts = email.trim().split("@");
  if (parts.length !== 2) return null;
  const [local, domain] = parts;
  const domainLower = domain.toLowerCase();

  if (POPULAR_EMAIL_DOMAINS.includes(domainLower)) return null;

  for (const popular of POPULAR_EMAIL_DOMAINS) {
    const distance = levenshteinDistance(domainLower, popular);
...
export function getLraIssues(form: LraFormState): string[] {
  ...
  if (!form.email.trim()) {
    ...
    const suggestion = findLikelyDomainTypo(form.email.trim());
    if (suggestion) {
      issues.push(`Did you mean ${suggestion}? Please double-check your email for typos.`);
    }
  }
...
const LS_FORM      = "peso_lra_draft";
const LS_SUBMITTED = "peso_lra_submitted";

const EMPTY_FORM: LraFormState = {
  lraId:                 "",
  lraName:               "",
  lraDate:               "",
  lraTime:               "",
...
function loadForm(): LraFormState {
  try {
    ...
    return { ...EMPTY_FORM, ...(JSON.parse(raw) as Partial<LraFormState>) };
  } catch { return EMPTY_FORM; }
}

function saveForm(form: LraFormState): void {
  try {
    const safeToStore: Partial<LraFormState> = { ...form };
...
export default function LraSchedules() {
  const navigate = useNavigate();

  const [form, setForm]                   = useState<LraFormState>(() => loadForm());
  ...
  const handleSelectLra = (event: { id: string; name: string; date: string; time: string; venue: string }) => {
    setForm(prev => ({
      ...prev,
      lraId:    event.id,
      lraName:  event.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = buildLraPayload(form, verificationToken);
    console.log("Submitting LRA payload:", JSON.stringify(payload, null, 2));

    const result = await submitLra(form, verificationToken);
    setSubmitting(false);

    console.log("LRA submit result:", result);
