...
import {
  LRA_EVENTS,
  LRA_JOB_LISTINGS,
  LRA_EDUCATION_LEVELS,
  LRA_DEGREE_OPTIONS,
  LRA_GENDER_OPTIONS,
  LRA_CIVIL_OPTIONS,
  LRA_DISABILITY_OPTIONS,
  LRA_EMPLOYMENT_OPTIONS,
  LRA_OFW_OPTIONS,
  LRA_FOURPS_OPTIONS,
  LRA_LANGUAGE_OPTIONS,
  LRA_PLACEHOLDER_COLORS,
  LRA_SUBMIT_URL,
  isApplicationClosed,
  formatDeadline,
  getTimeRemaining,
...
export interface LraSchedulesDesignProps {
  form: LraFormState;
  issues: string[];
  attempted: boolean;
  submitting: boolean;
  ...
  onUpdateForm: (patch: Partial<LraFormState>) => void;
  onToggleLanguage: (lang: string) => void;
  onSelectLra: (event: { id: string; name: string; date: string; time: string; venue: string }) => void;
  onBackToEvents: () => void;
...
function DegreeCombobox({ value, onChange, hasError = false }: DegreeComboboxProps) {
  ...
  useEffect(() => {
    if (value && !LRA_DEGREE_OPTIONS.includes(value)) {
      setOwnMode(true);
      setOwnText(value);
    }
  ...
  const filtered = query.trim()
    ? LRA_DEGREE_OPTIONS.filter(d => d.toLowerCase().includes(query.toLowerCase()))
    : LRA_DEGREE_OPTIONS;
  ...
  const isPreset = LRA_DEGREE_OPTIONS.includes(value);
...
// useEmailVerification hook that Emailverificationgate.tsx uses — so SRA and
// any other consumer of the full-page gate are completely unaffected.
//
...
// to a clean idle state. Ported directly from JobFairSchedulesDesign.tsx.
...
// Ported from AttendanceDesign.tsx.
...
// Ported from AttendanceDesign.tsx's RegistrationClosed, adapted to list LRA
// events instead of job fairs.
      ...
      <div style={{ minHeight: "100vh", background: "#fdf8f0", display: "flex", flexDirection: "column" }}>
        <header style={{ background: COLORS.red, padding: "12px 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, minHeight: 34 }}>
            ...
            <span style={{ color: "rgba(255,255,255,0.85)", fontSize: "0.85rem", fontWeight: 600 }}>LRA Pre-Registration</span>
          </div>
        </header>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "40px 20px" : "60px 24px" }}>
          <div style={{ background: "white", borderRadius: 16, boxShadow: "0 12px 48px rgba(26,29,94,0.10)", padding: isMobile ? "40px 28px 36px" : "56px 64px 52px", maxWidth: 560, width: "100%", textAlign: "center", position: "relative", animation: "fadeUp 0.4s ease both" }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: isMobile ? "1.5rem" : "1.9rem", color: COLORS.navy, lineHeight: 1.2, marginBottom: 14 }}>
              Pre-registration for these recruitment activities has ended.
            </h1>
            <p style={{ color: COLORS.bodyText, fontSize: "0.92rem", lineHeight: 1.65, marginBottom: 28 }}>
              ...
              Please visit the PESO Capiz office or check back for future recruitment activity announcements.
            </p>
            <div style={{ background: "rgba(26,29,94,0.04)", border: "1.5px solid rgba(26,29,94,0.09)", borderRadius: 10, padding: "16px 20px", marginBottom: 28, textAlign: "left" }}>
              <p style={{ fontSize: "0.7rem", fontWeight: 800, letterSpacing: 2, textTransform: "uppercase", color: COLORS.navy, marginBottom: 12 }}>
                Recruitment activities that were open for registration
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {LRA_EVENTS.map(event => (
                  <div key={event.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                    <div>
                      <span style={{ color: COLORS.navy, fontWeight: 700, fontSize: "0.92rem", display: "block" }}>{event.name}</span>
                      <span style={{ color: COLORS.bodyText, fontSize: "0.82rem" }}>{event.date}&nbsp;•&nbsp;{event.venue}</span>
                    </div>
...
// ── LRA Event Selection Screen ────────────────────────────────────────────────

function LraSelectionScreen({
  isMobile,
  onSelectLra,
}: {
  isMobile: boolean;
  ...
  const [lightboxEvent, setLightboxEvent] = useState<LraEvent | null>(null);
  ...
  return (
    <div style={{ maxWidth: 1320, margin: "0 auto", padding: isMobile ? "32px 18px" : "52px 24px" }}>
      ...
      <div style={{
        ...
        {LRA_EVENTS.map((event, i) => (
          <LraEventCard
            key={event.id}
            event={event}
            index={i}
            isMobile={isMobile}
            onSelect={onSelectLra}
            onViewImage={() => setLightboxEvent(event)}
          />
        ))}
      </div>
...
function LraEventCard({ event, index, isMobile, onSelect, onViewImage }: {
  event: LraEvent;
  index: number;
  isMobile: boolean;
  onSelect: (event: { id: string; name: string; date: string; time: string; venue: string }) => void;
  onViewImage: () => void;
}) {
  const [hovered, setHovered]         = useState(false);
  const [imageOk, setImageOk]         = useState(true);
  const [imageLoaded, setImageLoaded] = useState(false);
  const placeholder = LRA_PLACEHOLDER_COLORS[event.id] ?? { bg: COLORS.navy, accent: COLORS.gold };

  const showImage = !!event.image && imageOk;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={canExpand ? `View full image of ${event.name}` : undefined}
      >
        {showImage && (
          <img
            src={event.image!}
      ...
      <div style={{ padding: isMobile ? "20px 20px 22px" : "28px 30px 30px" }}>
        ...
        <button
          onClick={() => onSelect({ id: event.id, name: event.name, date: event.date, time: event.time, venue: event.venue })}
...
function SuccessPage({ form, isMobile, onNavigateHome }: {
  form: LraFormState; isMobile: boolean;
  onNavigateHome: () => void;
}) {
  return (
          ...
          <div style={{ flex: "1 1 440px", width: "100%", display: "flex", flexDirection: "column", gap: isMobile ? 18 : 26 }}>
            ...
            <div style={{ background: "rgba(26,29,94,0.03)", border: "1.5px solid rgba(26,29,94,0.09)", borderRadius: 14, padding: isMobile ? "20px 20px" : "26px 30px", textAlign: "left" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                {([
                  ["Recruitment Activity", form.lraName],
                  ["Date",                form.lraDate],
                  ["Time",                form.lraTime],
                  ["Venue",               form.lraVenue],
            ...
            <div style={{ background: "rgba(245,200,66,0.10)", border: "1.5px solid rgba(245,200,66,0.35)", borderRadius: 14, padding: isMobile ? "18px 20px" : "24px 30px", textAlign: "left" }}>
              ...
              <ul style={{ margin: 0, paddingLeft: 20, display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  "Please take a screenshot of this confirmation and present it at the registration area on the day of the recruitment activity as proof of your pre-registration.",
                ].map(r => (
...
export default function LraSchedulesDesign({
  ...
  onSelectLra, onBackToEvents, onEmailVerified, onChangeEmail, onSubmit, onNavigateHome,
}: LraSchedulesDesignProps) {
  ...
  // Checks for EXACT issue messages returned by getLraIssues, rather than loose
  // substring keywords. Substring matching previously caused false positives —
  // e.g. hasIssue("address") also matched the word "addresses" inside the
  // disposable-email message, which incorrectly put the Address field into an
  // error state whenever the Email field had an unrelated error.
  ...
  const educationOptions = LRA_EDUCATION_LEVELS.map(lvl => ({
    value: lvl.label,
    label: lvl.label,
  }));

  const lraSelected = !!form.lraId;
      ...
      <div style={{ minHeight: "100vh", background: "#fdf8f0", padding: "0 0 80px" }}>
        ...
        <header style={{ background: COLORS.red, padding: isMobile ? "10px 14px" : "12px 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", gap: isMobile ? 8 : 12, minHeight: 34, flexWrap: "wrap" }}>
            ...
            {lraSelected && !isMobile && (
              <>
                <span style={{ color: "rgba(255,255,255,0.5)" }}>/</span>
                <span style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.82rem", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>{form.lraName}</span>
              </>
            )}
          </div>
          {isMobile && (
            <div style={{ maxWidth: 1180, margin: "4px auto 0" }}>
              <span style={{ color: "rgba(255,255,255,0.8)", fontSize: "0.74rem", fontWeight: 600, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                LRA Pre-Registration{lraSelected ? ` · ${form.lraName}` : ""}
              </span>
            </div>
          )}
        ...
        {submitSuccess ? (
          ...
          <LraSelectionScreen isMobile={isMobile} onSelectLra={onSelectLra} />
        ) : (
              ...
              <div ref={formTopRef} style={{ flex: isMobile ? "none" : "1 1 0%", order: isMobile ? 2 : 1 }}>
                <div style={{ background: "white", borderRadius: isMobile ? 12 : 14, boxShadow: "0 8px 32px rgba(26,29,94,0.08)", padding: isMobile ? "18px 16px" : "32px 36px", animation: "slideIn 0.3s ease both" }}>
                  ...
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(26,29,94,0.05)", border: "1.5px solid rgba(26,29,94,0.12)", borderRadius: 99, padding: "5px 14px", marginBottom: 18, maxWidth: "100%" }}>
                    ...
                    <span style={{ fontSize: "0.88rem", fontWeight: 700, color: COLORS.navy, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{form.lraName}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: isMobile ? 10 : 20, flexWrap: "wrap", marginBottom: 24, padding: isMobile ? "10px 14px" : "12px 16px", background: "rgba(26,29,94,0.03)", border: "1.5px solid rgba(26,29,94,0.08)", borderRadius: 10 }}>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>📅 <strong style={{ color: COLORS.navy }}>{form.lraDate}</strong></span>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>🕗 <strong style={{ color: COLORS.navy }}>{form.lraTime}</strong></span>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>📍 <strong style={{ color: COLORS.navy }}>{form.lraVenue}</strong></span>
                  </div>

                  <span style={labelCaps}>LRA Pre-Registration</span>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      >
                        <InlineEmailVerification
                          key={emailWidgetKey}
                          formLabel={form.lraName || "LRA Registration"}
                          otpUrl={LRA_SUBMIT_URL}
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      <Field label="Gender" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Gender is required.") ? "Gender is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Gender is required."))} value={form.gender} onChange={e => onUpdateForm({ gender: e.target.value })}>
                          ...
                          {LRA_GENDER_OPTIONS.map(g => <option key={g} value={g}>{g}</option>)}
                        </select>
                      </Field>
                      <Field label="Civil Status" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Civil status is required.") ? "Civil status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Civil status is required."))} value={form.civilStatus} onChange={e => onUpdateForm({ civilStatus: e.target.value })}>
                          ...
                          {LRA_CIVIL_OPTIONS.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </Field>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      <Field label="Disability" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Please indicate if you have a disability.") ? "Please indicate if you have a disability." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Please indicate if you have a disability."))} value={form.hasDisability} onChange={e => onUpdateForm({ hasDisability: e.target.value, ...(e.target.value !== "Yes" ? { disabilityDetails: "" } : {}) })}>
                          ...
                          {LRA_DISABILITY_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                      </Field>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      <Field label="Employment Status" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Employment status is required.") ? "Employment status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Employment status is required."))} value={form.employmentStatus} onChange={e => onUpdateForm({ employmentStatus: e.target.value })}>
                          ...
                          {LRA_EMPLOYMENT_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                      <Field label="Are you an OFW?" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("OFW status is required.") ? "OFW status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("OFW status is required."))} value={form.ofwStatus} onChange={e => onUpdateForm({ ofwStatus: e.target.value })}>
                          <option value="">Select status</option>
                          {LRA_OFW_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                      <Field label="4Ps Beneficiary" required hint="Pantawid Pamilyang Pilipino Program" style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Please indicate if you are a 4Ps beneficiary.") ? "Please indicate if you are a 4Ps beneficiary." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Please indicate if you are a 4Ps beneficiary."))} value={form.fourPsBeneficiary} onChange={e => onUpdateForm({ fourPsBeneficiary: e.target.value })}>
                          <option value="">Select an option</option>
                          {LRA_FOURPS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      <Field label="School" required style={{ flex: isMobile ? "1 1 100%" : "1 1 240px" }} error={hasIssue("School name is required.") ? "School name is required." : undefined}>
                        <input style={fieldInputStyle(hasIssue("School name is required."))} value={form.school} onChange={e => onUpdateForm({ school: e.target.value.toUpperCase() })} placeholder="E.G. FILAMER CHRISTIAN UNIVERSITY" />
                      </Field>
