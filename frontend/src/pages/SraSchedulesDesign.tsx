...
import {
  SRA_EVENTS,
  SRA_JOB_LISTINGS,
  SRA_EDUCATION_LEVELS,
  SRA_DEGREE_OPTIONS,
  SRA_GENDER_OPTIONS,
  SRA_CIVIL_OPTIONS,
  SRA_DISABILITY_OPTIONS,
  SRA_EMPLOYMENT_OPTIONS,
  SRA_OFW_OPTIONS,
  SRA_FOURPS_OPTIONS,
  SRA_LANGUAGE_OPTIONS,
  SRA_PLACEHOLDER_COLORS,
  SRA_SUBMIT_URL,
  isApplicationClosed,
  formatDeadline,
  getTimeRemaining,
...
export interface SraSchedulesDesignProps {
  form: SraFormState;
  issues: string[];
  attempted: boolean;
  submitting: boolean;
  ...
  onUpdateForm: (patch: Partial<SraFormState>) => void;
  onToggleLanguage: (lang: string) => void;
  onSelectSra: (event: { id: string; name: string; date: string; time: string; venue: string }) => void;
  onBackToEvents: () => void;
...
function DegreeCombobox({ value, onChange, hasError = false }: DegreeComboboxProps) {
  ...
  useEffect(() => {
    if (value && !SRA_DEGREE_OPTIONS.includes(value)) {
      setOwnMode(true);
      setOwnText(value);
    }
  ...
  const filtered = query.trim()
    ? SRA_DEGREE_OPTIONS.filter(d => d.toLowerCase().includes(query.toLowerCase()))
    : SRA_DEGREE_OPTIONS;
  ...
  const isPreset = SRA_DEGREE_OPTIONS.includes(value);
...
// useEmailVerification hook that Emailverificationgate.tsx uses — so any
// other consumer of the full-page gate is completely unaffected.
//
...
// to a clean idle state. Ported directly from LraSchedulesDesign.tsx /
// JobFairSchedulesDesign.tsx.
...
// Ported from AttendanceDesign.tsx's RegistrationClosed, adapted to list SRA
      ...
      <div style={{ minHeight: "100vh", background: "#fdf8f0", display: "flex", flexDirection: "column" }}>
        <header style={{ background: COLORS.red, padding: "12px 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, minHeight: 34 }}>
            ...
            <span style={{ color: "rgba(255,255,255,0.85)", fontSize: "0.85rem", fontWeight: 600 }}>SRA Pre-Registration</span>
          </div>
        </header>
...
// ── SRA Event Selection Screen ────────────────────────────────────────────────

function SraSelectionScreen({
  isMobile,
  onSelectSra,
}: {
  isMobile: boolean;
  ...
  const [lightboxEvent, setLightboxEvent] = useState<SraEvent | null>(null);
  ...
  return (
    <div style={{ maxWidth: 1320, margin: "0 auto", padding: isMobile ? "32px 18px" : "52px 24px" }}>
      ...
      <div style={{
        ...
        {SRA_EVENTS.map((event, i) => (
          <SraEventCard
            key={event.id}
            event={event}
            index={i}
            isMobile={isMobile}
            onSelect={onSelectSra}
...
function SraEventCard({ event, index, isMobile, onSelect, onViewImage }: {
  event: SraEvent;
  index: number;
  isMobile: boolean;
  ...
  const placeholder = SRA_PLACEHOLDER_COLORS[event.id] ?? { bg: COLORS.navy, accent: COLORS.gold };
...
// LraSchedulesDesign / JobFairSchedulesDesign / Jobs reference panels.
...
function ReferenceJobRow({ job, search, isMobile, isFirst }: {
  job: typeof SRA_JOB_LISTINGS[number];
  search: string;
  isMobile: boolean;
  isFirst: boolean;
}) {
...
function AvailableJobsReference({ isMobile }: { isMobile: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return SRA_JOB_LISTINGS;
    return SRA_JOB_LISTINGS.filter(j =>
...
function SuccessPage({ form, isMobile, onNavigateHome }: {
  form: SraFormState; isMobile: boolean;
  onNavigateHome: () => void;
}) {
  return (
          ...
          <div style={{ flex: "1 1 440px", width: "100%", display: "flex", flexDirection: "column", gap: isMobile ? 18 : 26 }}>
            ...
            <div style={{ background: "rgba(26,29,94,0.03)", border: "1.5px solid rgba(26,29,94,0.09)", borderRadius: 14, padding: isMobile ? "20px 20px" : "26px 30px", textAlign: "left" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                {([
                  ["Recruitment Activity", form.sraName],
                  ["Date",                form.sraDate],
                  ["Time",                form.sraTime],
                  ["Venue",               form.sraVenue],
...
export default function SraSchedulesDesign({
  ...
  onSelectSra, onBackToEvents, onEmailVerified, onChangeEmail, onSubmit, onNavigateHome,
}: SraSchedulesDesignProps) {
  ...
  // Checks for EXACT issue messages returned by getSraIssues, rather than
  ...
  const educationOptions = SRA_EDUCATION_LEVELS.map(lvl => ({
    value: lvl.label,
    label: lvl.label,
  }));

  const sraSelected = !!form.sraId;
      ...
      <div style={{ minHeight: "100vh", background: "#fdf8f0", padding: "0 0 80px" }}>
        ...
        <header style={{ background: COLORS.red, padding: isMobile ? "10px 14px" : "12px 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", alignItems: "center", gap: isMobile ? 8 : 12, minHeight: 34, flexWrap: "wrap" }}>
            ...
            {sraSelected && !isMobile && (
              <>
                <span style={{ color: "rgba(255,255,255,0.5)" }}>/</span>
                <span style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.82rem", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>{form.sraName}</span>
              </>
            )}
          </div>
          {isMobile && (
            <div style={{ maxWidth: 1180, margin: "4px auto 0" }}>
              <span style={{ color: "rgba(255,255,255,0.8)", fontSize: "0.74rem", fontWeight: 600, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                SRA Pre-Registration{sraSelected ? ` · ${form.sraName}` : ""}
              </span>
            </div>
          )}
        ...
        {submitSuccess ? (
          ...
          <SraSelectionScreen isMobile={isMobile} onSelectSra={onSelectSra} />
        ) : (
              ...
              <div ref={formTopRef} style={{ flex: isMobile ? "none" : "1 1 0%", order: isMobile ? 2 : 1 }}>
                <div style={{ background: "white", borderRadius: isMobile ? 12 : 14, boxShadow: "0 8px 32px rgba(26,29,94,0.08)", padding: isMobile ? "18px 16px" : "32px 36px", animation: "slideIn 0.3s ease both" }}>
                  ...
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(26,29,94,0.05)", border: "1.5px solid rgba(26,29,94,0.12)", borderRadius: 99, padding: "5px 14px", marginBottom: 18, maxWidth: "100%" }}>
                    ...
                    <span style={{ fontSize: "0.88rem", fontWeight: 700, color: COLORS.navy, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{form.sraName}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: isMobile ? 10 : 20, flexWrap: "wrap", marginBottom: 24, padding: isMobile ? "10px 14px" : "12px 16px", background: "rgba(26,29,94,0.03)", border: "1.5px solid rgba(26,29,94,0.08)", borderRadius: 10 }}>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>📅 <strong style={{ color: COLORS.navy }}>{form.sraDate}</strong></span>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>🕗 <strong style={{ color: COLORS.navy }}>{form.sraTime}</strong></span>
                    <span style={{ fontSize: isMobile ? "0.78rem" : "0.83rem", color: COLORS.bodyText }}>📍 <strong style={{ color: COLORS.navy }}>{form.sraVenue}</strong></span>
                  </div>

                  <span style={labelCaps}>SRA Pre-Registration</span>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      >
                        <InlineEmailVerification
                          key={emailWidgetKey}
                          formLabel={form.sraName || "SRA Registration"}
                          otpUrl={SRA_SUBMIT_URL}
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      ...
                      <Field label="Gender" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Gender is required.") ? "Gender is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Gender is required."))} value={form.gender} onChange={e => onUpdateForm({ gender: e.target.value })}>
                          ...
                          {SRA_GENDER_OPTIONS.map(g => <option key={g} value={g}>{g}</option>)}
                        </select>
                      </Field>
                      <Field label="Civil Status" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Civil status is required.") ? "Civil status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Civil status is required."))} value={form.civilStatus} onChange={e => onUpdateForm({ civilStatus: e.target.value })}>
                          ...
                          {SRA_CIVIL_OPTIONS.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                      </Field>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      <Field label="Disability" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Please indicate if you have a disability.") ? "Please indicate if you have a disability." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Please indicate if you have a disability."))} value={form.hasDisability} onChange={e => onUpdateForm({ hasDisability: e.target.value, ...(e.target.value !== "Yes" ? { disabilityDetails: "" } : {}) })}>
                          ...
                          {SRA_DISABILITY_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                      </Field>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                      <Field label="Employment Status" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Employment status is required.") ? "Employment status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Employment status is required."))} value={form.employmentStatus} onChange={e => onUpdateForm({ employmentStatus: e.target.value })}>
                          ...
                          {SRA_EMPLOYMENT_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                      <Field label="Are you an OFW?" required style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("OFW status is required.") ? "OFW status is required." : undefined}>
                        <select style={fieldInputStyle(hasIssue("OFW status is required."))} value={form.ofwStatus} onChange={e => onUpdateForm({ ofwStatus: e.target.value })}>
                          <option value="">Select status</option>
                          {SRA_OFW_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                      <Field label="4Ps Beneficiary" required hint="Pantawid Pamilyang Pilipino Program" style={{ flex: isMobile ? "1 1 100%" : "1 1 180px" }} error={hasIssue("Please indicate if you are a 4Ps beneficiary.") ? "Please indicate if you are a 4Ps beneficiary." : undefined}>
                        <select style={fieldInputStyle(hasIssue("Please indicate if you are a 4Ps beneficiary."))} value={form.fourPsBeneficiary} onChange={e => onUpdateForm({ fourPsBeneficiary: e.target.value })}>
                          <option value="">Select an option</option>
                          {SRA_FOURPS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </Field>
                    </div>
