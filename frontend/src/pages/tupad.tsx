...
import tupadLogo from "/assets/tupad.png";
...
interface TupadPhoto {
  src: string;
  alt: string;
}

interface TupadFolder {
  id: string;
  name: string;
  photos: TupadPhoto[];
...
// e.g. { src: "/assets/tupad-orientation-1.jpg", alt: "Beneficiary orientation" }.
// Until then, `placeholderCount` controls how many empty slots show up so the
// folder doesn't look broken.

const TUPAD_FOLDERS: TupadFolder[] = [
  {
    id: "orientation",
    name: "TUPAD ORIENTATION & ENROLLMENT",
    photos: [],
    placeholderCount: 6,
  },
  {
    id: "clean-up-greening",
    name: "TUPAD IN ACTION",
    photos: [
      { src: "/assets/TUPAD/action/1.jpg", alt: "" },
      { src: "/assets/TUPAD/action/2.jpg", alt: "" },
      { src: "/assets/TUPAD/action/3.jpg", alt: "" },
      { src: "/assets/TUPAD/action/4.jpg", alt: "" },
      { src: "/assets/TUPAD/action/5.jpg", alt: "" },
      { src: "/assets/TUPAD/action/6.jpg", alt: "" },
      { src: "/assets/TUPAD/action/7.jpg", alt: "" },
      { src: "/assets/TUPAD/action/8.jpg", alt: "" },
    ],
    placeholderCount: 6,
  },
  {
...
function PhotoCell({
  ...
  photo: TupadPhoto;
  onOpenLightbox: (photo: TupadPhoto) => void;
}) {
...
export default function TupadPage() {
  ...
  const [lightbox, setLightbox] = useState<TupadPhoto | null>(null);
  ...
  const selectedFolder = TUPAD_FOLDERS.find(f => f.id === selectedFolderId) ?? null;
      ...
      <div style={{ minHeight: "100vh", background: COLORS.cream }}>
        ...
        <header style={{ background: COLORS.red, padding: "0 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto", height: 58, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ color: "rgba(255,255,255,0.85)", fontSize: "0.85rem", fontWeight: 600 }}>TUPAD Program</span>
          </div>
        </header>
          ...
          <div style={{
            ...
            <div style={{ flex: "1 1 400px", animation: "slideIn 0.4s ease both" }}>
              <div style={{
                ...
                <span style={{ fontSize: "0.68rem", fontWeight: 800, letterSpacing: 3, textTransform: "uppercase", color: COLORS.gold }}>
                  DOLE Emergency Employment Program
                </span>
              </div>
              ...
              <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "1rem", lineHeight: 1.75, maxWidth: 520, margin: "0 0 32px" }}>
                A DOLE community-based program that provides <strong style={{ color: "white" }}>short-term
                emergency employment</strong> to displaced, disadvantaged, and underemployed workers in the
                informal sector, engaging them in socially and economically useful community projects
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
                <img src={tupadLogo} alt="TUPAD" style={{ width: isMobile ? 300 : 420, height: isMobile ? 300 : 420, objectFit: "contain" }} />
                <span style={{ color: COLORS.gold, fontSize: "0.7rem", fontWeight: 800, letterSpacing: 3, textTransform: "uppercase", textAlign: "center" }}>
                  A DOLE Community-Based Emergency Employment Program
                </span>
              </div>
            </div>
          </div>
        ...
        <section style={{ background: COLORS.red, padding: isMobile ? "28px 24px" : "32px 24px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            <p style={{
              ...
              <strong>Program Objective:</strong> To provide immediate, short-term wage employment to
              disadvantaged and displaced workers whose livelihoods have been affected by crises or
              economic disruption, while engaging them in community projects that benefit their own
              neighborhoods.
            </p>
          </div>
        </section>
        ...
        <section style={{ padding: isMobile ? "40px 24px 0" : "56px 24px 0" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: isMobile ? 28 : 36 }}>
              ...
              <p style={{ color: COLORS.bodyText, fontSize: "0.9rem", maxWidth: 560, margin: "0 auto" }}>
                Priority is given to workers in the informal sector who lost income due to a crisis,
                displacement, or seasonal disruption.
              </p>
            </div>
            <div style={{
              display:             "grid",
              ...
              {[
                { icon: "🧑‍🌾", text: "Underemployed or self-employed workers, including those with elementary occupations" },
                { icon: "🏠", text: "Unpaid family workers and household helpers" },
                { icon: "🚚", text: "Displaced or income-affected drivers, vendors, and independent contractors" },
                { icon: "👴", text: "Senior citizens fit to work who are not already receiving a pension" },
                { icon: "🆔", text: "At least 18 years old and a resident of the endorsing barangay or LGU" },
                { icon: "🚫", text: "Not currently a beneficiary of another government emergency assistance program" },
              ].map((item, i) => (
                <div key={i} style={{
                  display:      "flex",
                  alignItems:   "flex-start",
                  ...
                  <span style={{ fontSize: "0.86rem", color: COLORS.bodyText, lineHeight: 1.5 }}>{item.text}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── What beneficiaries receive strip ── */}
        <section style={{ padding: isMobile ? "0 24px 0" : "0 24px 0" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: isMobile ? 28 : 36 }}>
              <span style={{ fontSize: "0.72rem", fontWeight: 800, letterSpacing: 4, textTransform: "uppercase", color: COLORS.red, display: "block", marginBottom: 10 }}>
                Program Benefits
            ...
            <div style={{
              ...
              {[
                { icon: "💵", title: "Daily Wage", text: "Pay based on the applicable regional minimum wage for the duration of the engagement" },
                { icon: "📅", title: "10–30 Days of Work", text: "Short-term engagement in a community project near the beneficiary's residence" },
                { icon: "🛡️", title: "Accident Insurance", text: "Personal accident insurance coverage for the duration of the program" },
                { icon: "🧰", title: "Work Materials", text: "Tools and materials needed to carry out the assigned community project" },
              ].map((item, i) => (
                <div key={i} style={{
                  background:   "white",
                  border:       "1.5px solid rgba(26,29,94,0.10)",
                  ...
                  <span style={{ fontSize: "1.5rem", display: "block", marginBottom: 10 }}>{item.icon}</span>
                  <h3 style={{ margin: "0 0 6px", fontSize: "0.88rem", fontWeight: 800, color: COLORS.navy }}>{item.title}</h3>
                  <p style={{ margin: 0, fontSize: "0.82rem", color: COLORS.bodyText, lineHeight: 1.5 }}>{item.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── Photo folders ── */}
        <section style={{ padding: isMobile ? "0 24px 64px" : "0 24px 88px" }}>
          <div style={{ maxWidth: 1180, margin: "0 auto" }}>
            ...
            {!selectedFolder && (
              <div style={{ textAlign: "center", marginBottom: isMobile ? 28 : 36 }}>
                ...
                <p style={{ color: COLORS.bodyText, fontSize: "0.95rem", maxWidth: 480, margin: "0 auto" }}>
                  Browse TUPAD highlights by folder. Tap a folder to view its photos.
                </p>
              </div>
            )}
        ...
        <section style={{
          ...
          <div style={{ maxWidth: 600, margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: isMobile ? "1.5rem" : "1.9rem", color: "white", margin: "0 0 12px", lineHeight: 1.2 }}>
              Interested in the TUPAD Program?
            </h2>
            <p style={{ color: "rgba(255,255,255,0.65)", fontSize: "0.95rem", lineHeight: 1.7, margin: "0 0 28px" }}>
              Coordinate through your barangay or the PESO Capiz office to check enrollment
